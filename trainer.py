...
from models.clip_loss import calculate_clip_loss
...
class TrainerBase:
    ...
    def print_model_info(self):
        if self.rank==0:
            ...
            aaa=count_params(self.model)
            self.logger.info(f"Number of parameters222222aaaaaaaa: {aaa:.2f}M")
...
class TrainerDifIR(TrainerBase):
    def __init__(self, configs):
        ...
        clip_ckpt_path = self.configs.get('clip', {}).get('ckpt_path')
        self.clip_model, self.preprocess = clip.load(clip_ckpt_path, device=self.device)
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan and phase == 'train':
            ...
            return {'lq':self.lq, 'gt':self.gt, 'text': text}
        else:
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            text_batch=[]
            text_path = self.configs.get('clip', {}).get('text_emb_path')
            text_names = [str(int(value.item())) + '.npy' for value in micro_data['text']]
            text_file_paths = [os.path.join(text_path, name) for name in text_names]
            for i in range(len(text_file_paths)):
                text_file_path=text_file_paths[i]
                select_embedding=np.load(text_file_path)
                select_embedding=np.squeeze(select_embedding)
                text_batch.append(select_embedding)
            text_batch=np.array(text_batch)
            text_batch = torch.tensor(text_batch,device= micro_data['gt'].device, dtype=torch.float32)


            text_batch_org=[]
            text_path_org = self.configs.get('clip', {}).get('text_org_path')
            text_names_org = [str(int(value.item())) + '.txt' for value in micro_data['text']]
            text_file_paths_org = [os.path.join(text_path_org, name) for name in text_names_org]
            for i in range(len(text_file_paths_org)):
                text_file_path_org=text_file_paths_org[i]
                with open(text_file_path_org, 'r') as f:
                    text_descriptions = f.readlines()
                    selected_text = random.choice(text_descriptions)
                    # print("selected_text:",selected_text)
                    text_batch_org.append(selected_text)
...
# get image
                x0_pred_toclip = self.base_diffusion.decode_first_stage(
                    self.base_diffusion._scale_input(z0_pred, tt),
                    self.autoencoder,
                )

                # x0_pred_toclip = vutils.make_grid(x0_pred_toclip, normalize=True)
                image_list=[]
                for i in range(x0_pred_toclip.size(0)):
                    image_tensor = x0_pred_toclip[i]

                    image_pil = to_pil_image(image_tensor)
                    image_list.append(image_pil)
                clip_loss=calculate_clip_loss(self.clip_model,self.preprocess ,image_list,text_batch_org,device=micro_data['gt'].device)

                loss = (losses["loss"]+clip_loss).mean() / num_grad_accumulate
                print(losses["loss"])
                loss.backward()
    ...
    def validation(self, phase='val'):
        if self.rank==0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                if 'gt' in data:
                    im_lq, im_gt,im_text = data['lq'], data['gt'],data['text']
                else:
                    im_lq,im_text = data['lq'],data['text']
                num_iters = 0
                ...
                text_path = self.configs.get('clip', {}).get('text_emb_path')
                text_names = [str(int(value.item())) + '.npy' for value in im_text]
                text_file_paths = [os.path.join(text_path, name) for name in text_names]
                text_batch = []
                for i in range(len(text_file_paths)):
                    text_file_path = text_file_paths[i]
                    ...
                    text_batch.append(select_embedding)
                text_batch = np.array(text_batch)
                text_batch=torch.from_numpy(text_batch).to(dtype=torch.float32).cuda()
