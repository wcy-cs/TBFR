...
class ResShiftSampler(BaseSampler):
    def __init__(self, configs, clip_ckpt_path=None, text_path=None, **kwargs):
        super().__init__(configs, **kwargs)
        self.clip_ckpt_path = clip_ckpt_path
        self.text_path = text_path 
        device = torch.device(f"cuda:{self.rank}" if torch.cuda.is_available() else "cpu")
        self.clip_model, self.preprocess = clip.load(self.clip_ckpt_path, device=device)

    def sample_func(self, y0,text_embedding, noise_repeat=False):
        '''
        Input:
    ...
    def inference(self, in_path, out_path,text_emb=None, bs=1, noise_repeat=False):
        '''
        Inference demo.
        Input:
        ...
        '''
        def _process_per_image(im_lq_tensor,text_embedding):
            '''
            Input:
            ...
            if im_lq_tensor.shape[2] > self.chop_size or im_lq_tensor.shape[3] > self.chop_size:
                ...
                for im_lq_pch, index_infos in im_spliter:
                    im_sr_pch = self.sample_func(
                            (im_lq_pch - 0.5) / 0.5,text_embedding,
                            noise_repeat=noise_repeat,
                            )     # 1 x c x h x w, [-1, 1]
                ...
                im_sr_tensor = self.sample_func(
                        (im_lq_tensor - 0.5) / 0.5,text_embedding,
                        noise_repeat=noise_repeat,
                        )     # 1 x c x h x w, [-1, 1]
        ...
        def tokenize_with_truncation(text: str, max_length: int = MAX_LENGTH):
            """Tokenize text with CLIP, truncating to `max_length` tokens."""
            return clip.tokenize(text, context_length=max_length, truncate=True)
        
        def embed_single_txt(txt_path: str) -> np.ndarray:
            ...
            with open(txt_path, "r", encoding="utf-8") as f:
                text = f.read()
            tokens = tokenize_with_truncation(text).to(device)
        
            # Encode without gradients
            with torch.no_grad():
                embedding = model_longclip.encode_text(tokens).cpu().numpy()
        ...
        # print(bs)
        device = "cuda" if torch.cuda.is_available() else "cpu"
        model_longclip, preprocess = self.clip_model, self.preprocess
        ...
        if bs > 1:
            ...
            if not in_path.is_dir():
                ...
                text_batch = torch.from_numpy(text_emb).cuda()
                im_sr_tensor = _process_per_image(im_lq_tensor,text_batch)
                ...
                for im_path in im_path_list:
                    ...
                    text_name = os.path.basename(im_path)
                    text_path = (text_path + text_name).replace('.png', '.txt')
                    
                    text_batch = []
                    select_embedding = embed_single_txt(text_path)
