...
def get_named_eta_schedule(
    ...
    if schedule_name=='exponential':
        ...
        increaser = math.exp(1 / (num_diffusion_timesteps - 1) * math.log(etas_end / etas_start))  # b0
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        # print("cond_stage_config:", cond_stage_config)
    ...
    def instantiate_cond_stage(self, config):
        ...
        model = util_common.instantiate_from_config(config)
    ...
    def p_mean_variance(
        ...
        model_output = model(self._scale_input(x_t, t), t,text, **model_kwargs)
    ...
    def p_sample(self, model, x, y, t, text=None,clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        """
        ...
        :return: a dict containing the following keys:
                 - 'sample': a random sample from the model. x_{t-1}
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"],
                "mean": out["mean"]}  # shap torch.Size([1, 3, 64, 64])
    ...
    def p_sample_loop_progressive(
        ...
        # print("p_sample_loop_progressive : shape of y:", y.shape)
        z_y = self.encode_first_stage(y, first_stage_model, up_sample=True)
        # print("p_sample_loop_progressive : shape of z_y:", z_y.shape)
        ...
        # print("p_sample_loop_progressive : shape of z_sample:", z_sample.shape)  # 64
    ...
    def get_learned_conditioning(self, c):
        ...
        if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
            ...
            print("WARNING: cond_stage_forward is not defined, using encode instead.")
    ...
    def training_losses(
        ...
        :param x_start: the [N x C x ...] tensor of inputs.   micro_data['gt'],
        :param y: the [N x C x ...] tensor of degraded inputs.    micro_data['lq'],
        ...
        text_emb = None
        if cond_text:
            # text_emb = self.get_learned_conditioning(text)
            # text_emb = th.from_numpy(text).to(z_y.device)
            text_emb=text
        ...
        z_t = self.q_sample(z_start, z_y, t, noise=noise)  # x_t
       

        terms = {}

        if self.loss_type==LossType.MSE or self.loss_type==LossType.WEIGHTED_MSE:
            # model :models.unetText.UNetModel
            # print(model_kwargs.keys())
            model_output = model(self._scale_input(z_t, t), t, text_emb,**model_kwargs)
            target = {
