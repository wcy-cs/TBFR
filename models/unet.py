...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        if cond_lq and lq_size == image_size:
            ...
            # feature_chn = 3
            # base_chn = 16
            # for ii in range(int(math.log(image_size/lq_size) / math.log(2))):
            #     feature_extractor.append(nn.Conv2d(feature_chn, base_chn, 3, 1, 1))
            #     feature_extractor.append(nn.SiLU())
            #     feature_extractor.append(Upsample(base_chn, True, out_channels=base_chn//2))
            #     base_chn //= 2
            #     feature_chn = base_chn
            # self.feature_extractor = nn.Sequential(*feature_extractor)
    ...
    def forward(self, x, timesteps,content, lq=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
