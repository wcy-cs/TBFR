...
def create_dataset(dataset_config):
    ...
    elif dataset_config['type'] == 'blinddata':
        dataset = BlindData(**dataset_config['params'])    
    else:
...
class BicubicData(Dataset):
    def __init__(
            ...
            sf,
            dir_path=None,
            text_path='/home/t2f/data/dataset/text/text/celeba-caption/',
            txt_file_path=None,
            mean=0.5,
            std=0.5,
            hflip=False,
        ...
        self.text_data = {}  # Store text data corresponding to each image
        for file_path in self.file_paths:
            file_name=os.path.basename(file_path)
            text_name = file_name.replace('.png', '')

            text_name=int(text_name)
            text_name=torch.tensor(text_name)
            self.text_data[file_path] = text_name
            ...
            #     self.text_data[file_path] = selected_text.strip()
    ...
    def __getitem__(self, index):
        ...
        # print("image_lq.shape",im_lq.shape)
        im_lq = np.clip(im_lq, 0.0, 1.0)
        
        # print(type(self.text_data[im_path]))
        out = {'lq':self.transform(im_lq), 'gt':self.transform(im_gt), 'text': self.text_data[im_path]}
...
class BlindData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale_size = np.random.randint(4, 64)

        temp = Image.fromarray((im_gt * 255).astype(np.uint8))
        im_lq = complex_imgaug(temp, 256, scale_size)
        im_lq = np.ascontiguousarray(im_lq)/ 255.0
        im_gt = np.ascontiguousarray(im_gt)

        im_lq = np.clip(im_lq, 0.0, 1.0)

        im_lq=ToTensor()(im_lq)
        im_gt=ToTensor()(im_gt)
        normalize(im_lq, self.mean, self.std, inplace=True)
        normalize(im_gt, self.mean, self.std, inplace=True)

        out = {'lq':im_lq, 'gt':im_gt,'text': self.text_data[im_path]}
...
def complex_imgaug(x, org_size, scale_size):
    """Apply blind degradation to an input image."""
    x = np.array(x)
    x = x[np.newaxis, :, :, :]
